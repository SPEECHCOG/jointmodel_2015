    ...
    %% Run experiment
        ...
        for cond = 1:2              % Two conditions: synch and non-synch
            ...
            for k = 1:length(bounds)
                if(cond == 1)
                    training_vqs{k} = data_train{1}(bounds(k,1):bounds(k,2));
                elseif(cond == 2)                    
            ...
            for viscond = 1:3
                ...
                elseif(viscond == 3) % Scrambled visuals
                    %tS = formatCMF(training_vqs,[randi(4,size(training_tags')) ones(size(training_tags')).*5],1:25,cbsize);
...
legend({'sync','async','adults'},'Location','NorthEast');
...
save(sprintf('results/thiessen_task_results_%d_new.mat',cbsize),'perf','res');
