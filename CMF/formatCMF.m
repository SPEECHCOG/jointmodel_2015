function tS = formatCMF(train_labels,tags_train,lags,maxtag,random)

tS.L = lags;

tS.train_labels = train_labels;
tS.tags_train = tags_train;

tS.alphabet_size = max(cellfun(@(x) max(x), train_labels));


%tS.number_of_tags = max(max(tags_train));
